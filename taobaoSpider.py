# -*- coding: utf-8 -*-
'''
https://github.com/jaxpz/sf_taobao-and-sf_jd/blob/master/taobaofapai.py
参考项目
...
def save_mysql(list, time_list, gets_url):
    ...
    # try:
    sql_2 = """
        INSERT IGNORE INTO sf_taobao (address,url,first_price,caution_money,evaluation_price,Price_increase,disposal_place,date_1)VALUES('{}','{}','{}','{}','{}','{}','{}','{}' )
            """ \
        .format(
        pymysql.escape_string(list[0]),
        pymysql.escape_string(gets_url[0]),
        pymysql.escape_string(list[2]),
        pymysql.escape_string(list[3]),
        pymysql.escape_string(list[4]),
        pymysql.escape_string(list[5]),
        pymysql.escape_string(list[6]),
        pymysql.escape_string(str(time_list[0])),
    )
    # print(sql_2)
    ...
    # list[0], gets_url[0], list[2], list[3], list[4], list[5], list[6], str(time_list[0])))
    # db.commit()
    print("插入成功")
    # except Exception as e:
    #     print("插入失败", e)
...
def add_set(list_url):
    ...
    with open('sf_taobao.txt', 'a+') as file_object:
...
def main():
    ...
    # 进入 https://sf.tapbao.com/item_list.htm 然后选择好属性之后 复制链接 即可
    # 全国房产 如果要筛选的话可以直接在数据中筛选地区为广州即可或者使用下面的链接
    url =  'https://sf.taobao.com/item_list.htm?spm=a213w.7398504.filter.2.335a6cb3kGhEj5&category=50025969&auction_source=0&sorder=1&st_param=-1&auction_start_seg=-1'
    # 广州房产
    url='https://sf.taobao.com/item_list.htm?spm=a213w.7398504.filter.2.335a6cb3kGhEj5&category=50025969&auction_source=0&city=%B9%E3%D6%DD&st_param=-1&auction_start_seg=-1'
